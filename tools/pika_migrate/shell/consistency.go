...
func main() {
	...
	// 添加连接超时配置
	masterOpts.DialTimeout = 5 * time.Second
	masterOpts.ReadTimeout = 3 * time.Second
	masterOpts.WriteTimeout = 3 * time.Second
	...
	for i, slave := range slaves {
		...
		// 添加连接超时配置
		slavesOpts[i].DialTimeout = 5 * time.Second
		slavesOpts[i].ReadTimeout = 3 * time.Second
		slavesOpts[i].WriteTimeout = 3 * time.Second
	}
	...
	// 测试主节点连接
	testClient := redis.NewClient(&masterOpts)
	ctx := context.Background()
	if err := testClient.Ping(ctx).Err(); err != nil {
		log.Fatalf("Failed to connect to master Redis at %s: %v", masterOpts.Addr, err)
	}
	log.Printf("Successfully connected to master Redis at %s", masterOpts.Addr)
	testClient.Close()
	
	// 测试从节点连接
	for _, slave := range slavesOpts {
		testSlaveClient := redis.NewClient(&slave)
		if err := testSlaveClient.Ping(ctx).Err(); err != nil {
			log.Fatalf("Failed to connect to slave Redis at %s: %v", slave.Addr, err)
		}
		log.Printf("Successfully connected to slave Redis at %s", slave.Addr)
		testSlaveClient.Close()
	}
...
func checkRoutine(sourceClient *redis.Client, targetClient []*redis.Client, cmds []string, keySpaceLen, dataSize, samplesPerClient uint64, keyPlace, dataPlace []int, seed int64, wg *sync.WaitGroup, dataPointsChan chan datapoint) {
	...
	if err := sourceClient.Ping(ctx).Err(); err != nil {
		log.Printf("Source client connection failed: %v", err)
		return
	}
	
	// 验证目标客户端连接
	for i, client := range targetClient {
		if err := client.Ping(ctx).Err(); err != nil {
			log.Printf("Target client %d connection failed: %v", i, err)
			return
		}
	}
